...
def display_results(scores, percentile, improvement_areas, brand_name="Unknown", industry="General", product_type="Product"):
    ...
    if st.session_state.use_openai and st.session_state.ai_insights:
        ...
        with ai_tab1:
            ...
            for strength in ai_insights.get("strengths", [])[:3]:
                ...
                <div style="background: white; border-radius: 8px; box-shadow: 0 1px 6px rgba(0,0,0,0.05); padding: 15px; margin-bottom: 15px;">
                    <div style="font-weight: 600; color: #10b981; margin-bottom: 8px;">{strength['area']}</div>
                    <div style="color: #333; font-size: 0.9rem;">{strength['explanation']}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            for trend in ai_insights.get("trends", [])[:3]:
                trends_html += f"""
                <div style="background: white; border-radius: 8px; box-shadow: 0 1px 6px rgba(0,0,0,0.05); padding: 15px; margin-bottom: 15px;">
                    <div style="font-weight: 600; color: #5865f2; margin-bottom: 8px;">{trend['trend']}</div>
                    <div style="color: #333; font-size: 0.9rem;">{trend['application']}</div>
                </div>
                """
            
            st.markdown(trends_html, unsafe_allow_html=True)
            
            # Hidden insights and prediction
            if "hidden_insight" in ai_insights and "performance_prediction" in ai_insights:
                st.markdown(f"""
        ...
        with ai_tab2:
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    <thead>
                        <tr style="background: #f8fafc; border-bottom: 2px solid #e2e8f0;">
                            <th style="padding: 12px 15px; text-align: left; font-weight: 600; color: #334155;">Competitor</th>
                            <th style="padding: 12px 15px; text-align: center; font-weight: 600; color: #334155;">Threat Level</th>
                            <th style="padding: 12px 15px; text-align: left; font-weight: 600; color: #334155;">Market Position</th>
                        </tr>
                    </thead>
                    <tbody>
            """
            
            # Competitor data and threat level indicators
            threat_colors = {
                "high": "#f43f5e",
                "medium": "#f97316", 
                "low": "#10b981"
            ...
            market_positions = [
                "Market leader in digital-first campaigns", 
                "Strong presence in emerging platforms",
                "Established player with traditional strength"
            ]
            
            for i, competitor in enumerate(competitor_analysis.get("competitors", [])[:3]):
                name = competitor.get("name", "Unnamed competitor")
                threat_level = competitor.get("threat_level", "medium").lower()
                color = threat_colors.get(threat_level, "#f97316")
                
                # For the market position, either generate based on threat level or use placeholder data
                pos_index = min(i, len(market_positions)-1)
                position = market_positions[pos_index]
                
                competitors_html += f"""
                <tr style="border-bottom: 1px solid #e2e8f0;">
                    <td style="padding: 15px; text-align: left; font-weight: 500;">{name}</td>
                    <td style="padding: 15px; text-align: center;">
                        <span style="background: {color}; color: white; padding: 5px 10px; border-radius: 4px; font-size: 0.8rem; font-weight: 500; text-transform: uppercase;">{threat_level}</span>
                    </td>
                    <td style="padding: 15px; text-align: left; color: #4b5563;">{position}</td>
                </tr>
                """
            ...
            # Create a row layout for advantages and threats
            col1, col2 = st.columns(2)
            ...
            with col1:
                ...
                for advantage in competitor_analysis.get("advantages", [])[:2]:
                    ...
                    <div style="background: white; border-radius: 8px; box-shadow: 0 1px 6px rgba(0,0,0,0.05); padding: 15px; margin-bottom: 15px; height: 100%;">
                        <div style="font-weight: 600; color: #10b981; margin-bottom: 8px;">{advantage.get('advantage', 'Strategic advantage')}</div>
                        <div style="color: #333; font-size: 0.9rem;">{advantage.get('explanation', 'No explanation provided')}</div>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            with col2:
                ...
                for threat in competitor_analysis.get("threats", [])[:2]:
                    st.markdown(f"""
                    <div style="background: white; border-radius: 8px; box-shadow: 0 1px 6px rgba(0,0,0,0.05); padding: 15px; margin-bottom: 15px; height: 100%;">
                        <div style="font-weight: 600; color: #f43f5e; margin-bottom: 8px;">{threat.get('threat', 'Market threat')}</div>
                        <div style="color: #333; font-size: 0.9rem; margin-bottom: 10px;">{threat.get('mitigation', 'No mitigation strategy provided')}</div>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            for opportunity in competitor_analysis.get("differentiation", [])[:2]:
                diff_html += f"""
                <div style="background: white; border-radius: 8px; box-shadow: 0 1px 6px rgba(0,0,0,0.05); padding: 15px; flex: 1; min-width: 300px;">
                    <div style="font-weight: 600; color: #5865f2; margin-bottom: 8px;">{opportunity.get('opportunity', 'Strategic opportunity')}</div>
                    <div style="color: #333; font-size: 0.9rem;">{opportunity.get('implementation', 'No implementation details provided')}</div>
                </div>
                """
        ...
        with ai_tab3:
            audience_segments = st.session_state.audience_segments
            
            if isinstance(audience_segments, dict) and "error" in audience_segments:
                st.warning(f"Unable to generate complete audience segmentation. Using limited data.")
                segments = audience_segments.get("segments", [])
            elif isinstance(audience_segments, list):
                segments = audience_segments[:3]  # Limit to 3 segments
            else:
                segments = []
            
            if segments:
                # Create a tabbed interface for the segments
                segment_tabs = st.tabs([segment.get('name', f"Segment {i+1}") for i, segment in enumerate(segments)])
                
                for i, (tab, segment) in enumerate(zip(segment_tabs, segments)):
                    with tab:
                        # Header with segment name and basic demographics
                        demographics = segment.get('demographics', {})
                        demo_html = f"""
                        <div style="margin-bottom: 25px;">
                            <h4 style="margin-bottom: 15px;">{segment.get('name', f'Segment {i+1}')}</h4>
                            <div style="display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 20px;">
                                <div style="background: #f0f2ff; border-radius: 6px; padding: 10px 15px;">
                                    <div style="font-size: 0.7rem; color: #5865f2; text-transform: uppercase; letter-spacing: 1px;">Age</div>
                                    <div style="font-weight: 600; color: #333;">{demographics.get('age_range', '25-45')}</div>
                                </div>
                                <div style="background: #f0f2ff; border-radius: 6px; padding: 10px 15px;">
                                    <div style="font-size: 0.7rem; color: #5865f2; text-transform: uppercase; letter-spacing: 1px;">Gender</div>
                                    <div style="font-weight: 600; color: #333;">{demographics.get('gender_skew', 'Balanced')}</div>
                                </div>
                                <div style="background: #f0f2ff; border-radius: 6px; padding: 10px 15px;">
                                    <div style="font-size: 0.7rem; color: #5865f2; text-transform: uppercase; letter-spacing: 1px;">Income</div>
                                    <div style="font-weight: 600; color: #333;">{demographics.get('income_level', 'Middle to upper-middle')}</div>
                                </div>
                                <div style="background: #f0f2ff; border-radius: 6px; padding: 10px 15px;">
                                    <div style="font-size: 0.7rem; color: #5865f2; text-transform: uppercase; letter-spacing: 1px;">Location</div>
                                    <div style="font-weight: 600; color: #333;">{demographics.get('location_type', 'Urban and suburban')}</div>
                                </div>
                            </div>
                        </div>
                        """
                        ...
                        # Create two columns for psychographics and media habits
                        col1, col2 = st.columns(2)
                        ...
                        with col1:
                            ...
                            if psychographics:
                                ...
                                for trait in psychographics:
                                    psycho_html += f"""
                                    <div style="background: white; border-radius: 8px; box-shadow: 0 1px 6px rgba(0,0,0,0.05); padding: 12px; margin-bottom: 10px;">
                                        <span style="color: #333;">{trait}</span>
                                    </div>
                                    """
                                psycho_html += "</div>"
                        ...
                        with col2:
                            ...
                            if media_habits:
                                ...
                                for habit in media_habits:
                                    channel = habit.get('channel', 'Unknown channel')
                                    affinity = habit.get('affinity', 'medium').lower()
                                    color = colors.get(affinity, "#3b82f6")
                                    
                                    media_html += f"""
                                    <div style="display: flex; justify-content: space-between; align-items: center; background: white; border-radius: 8px; box-shadow: 0 1px 6px rgba(0,0,0,0.05); padding: 12px; margin-bottom: 10px;">
                                        <span style="color: #333;">{channel}</span>
                                        <span style="background: {color}; color: white; padding: 3px 8px; border-radius: 4px; font-size: 0.7rem; text-transform: uppercase;">{affinity}</span>
                                    </div>
                                    """
                                media_html += "</div>"
                                st.markdown(media_html, unsafe_allow_html=True)
                        
                        # Cultural affinities and resonance factors
                        col1, col2 = st.columns(2)
                        ...
                        with col1:
                            ...
                            if cultural_affinities:
                                ...
                                for affinity in cultural_affinities:
                                    cultural_html += f"""
                                    <div style="background: #f0fdf4; border-radius: 8px; padding: 12px; margin-bottom: 10px; border-left: 3px solid #10b981;">
                                        <span style="color: #333;">{affinity}</span>
                                    </div>
                                    """
                                cultural_html += "</div>"
                            ...
                            if resonance_factors:
                                ...
                                for factor in resonance_factors:
                                    ...
                                    <div style="background: #eff6ff; border-radius: 8px; padding: 12px; margin-bottom: 10px; border-left: 3px solid #3b82f6;">
                                        <span style="color: #333;">{factor}</span>
                                    </div>
                                    """
                                resonance_html += "</div>"
                                st.markdown(resonance_html, unsafe_allow_html=True)
            else:
                st.info("Audience segmentation data not available. Please try running the analysis again.")
